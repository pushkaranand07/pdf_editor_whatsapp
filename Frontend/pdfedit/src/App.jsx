...
const PDFEditor = () => {
  ...
  const clearAnnotationsOnServer = async () => {
    ...
    try {
      ...
      console.error('Error clearing annotations on server:', error);
    }
  };
  ...
  const clearAnnotations = async () => {
    ...
    try {
      await clearAnnotationsOnServer();
  ...
  const handleMouseDown = (e) => {
    ...
    if (currentTool === 'eraser') {
      // For eraser, we'll use destination-out blend mode
      ctx.globalCompositeOperation = 'destination-out';
      ctx.beginPath();
      ctx.arc(x, y, penSize * 2, 0, 2 * Math.PI);
      ctx.fill();
      
      // Find and remove annotations that intersect with eraser position
      const eraserRadius = penSize * 2 / Math.min(imageDimensions.width, imageDimensions.height);
      const filteredAnnotations = annotations.filter(annotation => {
        if (!annotation.points || !annotation.points.length) return true;
        
        // Check if any point in the annotation is within eraser radius
        return !annotation.points.some(point => {
          const distance = Math.sqrt(
            Math.pow(point.x - imgX, 2) + Math.pow(point.y - imgY, 2)
          );
      ...
      if (filteredAnnotations.length !== annotations.length) {
        ...
        addToHistory(filteredAnnotations);
        // Save the updated annotations
        clearAnnotationsOnServer().then(() => {
          filteredAnnotations.forEach(annotation => saveAnnotation(annotation));
        });
      }
    } else {
  ...
  const handleMouseUp = () => {
    ...
    if (currentTool !== 'eraser' && annotations.length > 0) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans flex">
      {/* Sidebar */}
      <div className={`bg-white shadow-lg border-r border-gray-200 transition-all duration-300 ${showSidebar ? 'w-80' : 'w-0 overflow-hidden'}`}>
        <div className="p-6 h-full overflow-y-auto">
          <div className="mb-8">
            <h1 className="text-2xl font-extrabold text-gray-900 mb-1">AI PDF Editor</h1>
            <p className="text-sm text-gray-600">Smart annotation & analysis</p>
          </div>

          {/* File Upload */}
          <div className="mb-8">
            <h3 className="text-lg font-semibold mb-4 text-gray-900">Document</h3>
            <input
              type="file"
              ref={fileInputRef}
            ...
            <button
              ...
              className="w-full p-4 border-2 border-dashed border-gray-300 rounded-lg text-center hover:border-indigo-500 hover:bg-indigo-50 transition-all duration-150"
            >
              <Upload size={24} className="mx-auto mb-2 text-gray-400 group-hover:text-indigo-600" />
              <span className="text-gray-700">Click to upload PDF or image</span>
            </button>
              ...
              {totalPages > 0 && (
                <div className="mb-8">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-medium">{currentPage + 1} of {totalPages}</span>
                    <button 
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto">
                    {Array.from({ length: totalPages }, (_, i) => (
                      <div
                        key={i}
                        className={`aspect-[3/4] rounded border-2 cursor-pointer transition-all duration-200 ${i === currentPage ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300 bg-white'}`}
                        onClick={() => loadPage(i)}
                      >
      ...
      <div className="flex-1 flex flex-col">
            ...
            <div className="flex items-center gap-3">
              ...
              {fileName && (
                <div className="flex items-center gap-2">
                  <ToolButton icon={Undo} isActive={false} onClick={undo} tooltip="Undo" />
                  <ToolButton icon={Redo} isActive={false} onClick={redo} tooltip="Redo" />
                </div>
              )}
            </div>
            ...
            {fileName && (
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 bg-gray-100 rounded-lg p-1">
                  <button onClick={() => handleZoom('out')} className="p-2 rounded hover:bg-gray-200 transition-colors"><ZoomOut size={16} /></button>
                  <span className="text-sm font-medium min-w-[60px] text-center">{zoom}%</span>
                  <button onClick={() => handleZoom('in')} className="p-2 rounded hover:bg-gray-200 transition-colors"><ZoomIn size={16} /></button>
                  <button onClick={() => setZoom(100)} className="p-2 rounded hover:bg-gray-200 transition-colors"><RotateCcw size={16} /></button>
                </div>

                {/* Save Options */}
                <div className="flex items-center gap-2">
                  <ToolButton icon={Save} isActive={false} onClick={() => console.log('Save as PDF')} tooltip="Save PDF" />
                  <ToolButton icon={Image} isActive={false} onClick={saveAsImage} tooltip="Save Image" />
                  <ToolButton icon={Download} isActive={false} onClick={() => console.log('Download')} tooltip="Download" />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 flex items-center justify-center p-8 overflow-auto">
          ...
          >
            ...
            {uploadProgress > 0 && uploadProgress < 100 && (
              <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                <div className="bg-white rounded-lg p-6 text-center shadow-md">
                  ...
                  <p className="text-gray-800 font-medium">Loading document...</p>
            ...
            {/* Empty state */}
            {!fileName && (
              <div className="absolute inset-0 flex items-center justify-center text-center p-8 bg-transparent">
                <div>
                  <FileText size={64} className="mx-auto mb-4 text-gray-300" />
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">Upload a document to get started</h3>
                  <p className="text-gray-500 mb-4">Drag and drop or click to select PDF or image files</p>
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="px-6 py-3 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition-all duration-150 shadow"
                  >
                    Choose File
                  </button>
                </div>
        ...
        <div className="bg-white border-t border-gray-200 p-3">
          <div className="flex items-center justify-between text-sm text-gray-700">
            <div className="flex items-center gap-4">
              <span>Tool: <strong className="text-gray-900">{currentTool}</strong></span>
              <span>Color: <strong className="text-gray-900">{penColor}</strong></span>
              <span>Size: <strong className="text-gray-900">{penSize}px</strong></span>
              {screenshots.length > 0 && <span className="text-indigo-600">Screenshots: {screenshots.length}</span>}
            </div>
            <div className="flex items-center gap-4">
              {isRecording && (
              ...
              <span>Zoom: {zoom}%</span>
              {fileName && <span className="truncate max-w-xs">{fileName} (Page {currentPage + 1} of {totalPages})</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      {showPreviewModal && currentPreview && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowPreviewModal(false)}>
          <div className="bg-white rounded-lg p-6 max-w-2xl max-h-[80vh] overflow-auto shadow-lg" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Annotation Preview</h3>
              <button onClick={() => setShowPreviewModal(false)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">×</button>
            </div>
            
            <div className="mb-4">
              <img src={`data:image/png;base64,${currentPreview.image_base64}`} alt="Annotation preview" className="w-full rounded-lg border" />
            </div>
            
            <div className="space-y-2 text-sm text-gray-600">
              <div className="flex items-center gap-2">
                <span className="w-4 h-4 rounded-full" style={{ backgroundColor: currentPreview.color }} />
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => downloadScreenshot(currentPreview)} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">Download</button>
              <button onClick={() => setShowPreviewModal(false)} className="px-4 py-2 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
